...
function notificacao() {
    var assunto=" Unilurio Repository";
    var mensagem="foi adicionado um novo documento";

    $.ajax({
        url:"returnOllUser",
    }).done(function (data) {
        for (var i=0;i<data.length;i++){
             if(data.category==$('category').val()){
                 $.ajax({
                     url:"/sendMail/"+assunto+"/"+mensagem+"/"+data.email,
                 });
             }
        }
    })
...
function inserir_pesqisa() {
    ...
    $.ajax({
           ...
           url:"/recebeDadosAfiltrar/"+year+"/"+category+"/"+search,
         });
...
function addfile() {
    ...
    $('.modal-title').text('ANEXAR FICHEIROS'); // Set Title to Bootstrap modal title
}
...
$(document).ready(function () {
    ...
    function fill_datatable_files() {
        ...
        $.ajax({
            url:"/retornaDadosAfiltrar",
        }).done(function(data){
            if(data[0]==null && data[2]==1) {
                var year = $('#filter_year').val() != null ? $('#filter_year').val() : 'null';
                var category = $('#filter_category').val() != null ? $('#filter_category').val() : 'null';
                var search = $('#filter_name').val() == '' ? 'null' : $('#filter_name').val();
            }else {
